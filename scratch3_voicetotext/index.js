...
recognition.onresult = function (event) {
    console.time("語音辨識結果時間：");
    var i = event.resultIndex;
    var j = event.results[i].length - 1;
    v_textArray.pop();
    v_textArray.push(event.results[i][j].transcript);
    console.log('event:', v_textArray);
    console.timeEnd("語音辨識結果時間：");
};
...
class voicetoTEXT {
    ...
    beginVoice() {
        //	var recognition = new webkitSpeechRecognition();
        //	this.v_text.data= '';
        console.time("語音辨識開始至暫停辨識：");
        console.time("語音辨識開始：");
        recognition.start();
        console.timeEnd("語音辨識開始：");
    }

    pauseVoice() {
        console.time("暫停語音辨識：");
        recognition.abort();
        console.timeEnd("暫停語音辨識：");
        console.timeEnd("語音辨識開始至暫停辨識：");
    }
    ...
    nullTEXT() {
        ...
        recognition.abort();
        v_textArray[0] = "";
        this.v_text.data = v_textArray[0];
        recognition.start();
        console.timeEnd("清除語音文字：");
        return this.v_text.data;
    }

    voicetoTEXT() {
        console.time("顯示語音文字：");
        this.v_text.data = v_textArray[0];
        console.timeEnd("顯示語音文字：");
        return this.v_text.data;
    }
